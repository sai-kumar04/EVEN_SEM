...
class RCMControl:
  ...
  def RCMImpl(self, q):
    ...
    # J_2_inverse = np.linalg.pinv(J_ext[:3,:])
    J_1_inverse = np.linalg.pinv(J_ext[3:,:])

    J_2c = np.matmul(J_2, (np.identity(8) - np.matmul(J_1_inverse, J_rcm)))

    J_2c_inverse = np.linalg.pinv(J_2c)
    ...
    dq_task = np.matmul(J_2c_inverse, p_task - p_ip1)
    dq_rcm = np.matmul(J_1_inverse - np.matmul(np.matmul(J_2c_inverse, J_2),J_1_inverse), p_trocar - p_rcm)
    print(dq_task)
    print(dq_rcm)
